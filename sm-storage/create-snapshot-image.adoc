...
. 要创建快照映像、请执行以下操作之一：
+
** 从左侧面板中，选择菜单：Storage[Volumes]。选择对象（基本卷或快照一致性组），然后单击菜单：省略号图标[Create instant snapshot]。
** 从左侧面板中，选择菜单：Storage[Snapshots]。选择 *Snapshot Images* 选项卡，然后选择菜单：Create[Instant snapshot Image]。
+
