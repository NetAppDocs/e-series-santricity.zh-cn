...
. 选择*预留容量*选项卡。
. 选择要编辑的镜像对卷，然后单击菜单：省略号图标[Edit Settings]。
+
此时将显示 Edit Mirrored Pair Volume Reserved Capacity Settings（编辑镜像对卷保留容量设置）窗口。
...
预留容量已满警报阈值
 a| 
当镜像对的保留容量接近满时，使用条形图调整 System Manager 发送警报通知的百分比点。
