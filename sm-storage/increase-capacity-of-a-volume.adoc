...
. 选择要增加容量的卷，然后单击 menu:ellipsis icon[Increase Capacity]。
+
此时将显示确认增加容量页面。
...
+
此页面显示卷的当前报告容量和卷的关联池或卷组中的可用容量。

. 使用*通过添加...增加报告容量*框将容量添加到当前可用报告容量。您可以更改以 mebibytes (MiB)、gibibytes (GiB) 或 tebibytes (TiB) 显示的容量值。
. 单击 * 增加 * 。


.结果
* System Manager会根据您的选择增加卷的容量。
* 选择菜单：Storage[Operations in Progress] 以查看当前为所选卷运行的增加容量操作的进度。此操作可能很长，并可能影响系统性能。
