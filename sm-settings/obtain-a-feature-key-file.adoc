...
.步骤
. 在 System Manager 中，找到并记录机箱序列号。从左侧面板转到菜单：Support[Support Center]，然后向下滚动到 *View top storage array properties* 以查看机箱序列号。
. 在 System Manager 中，找到功能启用标识符。从左侧面板转到菜单：设置[系统]，然后向下滚动到*附加组件*。查找*功能启用标识符*。记录功能启用标识符的编号。
